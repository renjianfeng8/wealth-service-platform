...
public class GlobalExceptionHandler {
    ...
    public Result<?> handleServiceException(ServiceException e) {
        log.warn("[common:exception] 业务异常, code={}, message={}", e.getCode(), e.getMessage());
    ...
    public Result<?> handleValidationException(MethodArgumentNotValidException e) {
        ...
        log.warn("[common:exception] 参数校验失败, {}", message);
        return Result.error(400, "参数校验失败：" + message);
    }

    @ExceptionHandler(ConstraintViolationException.class)
    public Result<?> handleConstraintViolation(ConstraintViolationException e) {
        log.warn("[common:exception] 参数约束违例, {}", e.getMessage());
        return Result.error(400, "参数校验失败：" + e.getMessage());
    }

    @ExceptionHandler(MissingServletRequestParameterException.class)
    public Result<?> handleMissingParam(MissingServletRequestParameterException e) {
        log.warn("[common:exception] 缺少请求参数, {}", e.getMessage());
        return Result.error(400, "缺少请求参数：" + e.getParameterName());
    }

    @ExceptionHandler(MethodArgumentTypeMismatchException.class)
    public Result<?> handleTypeMismatch(MethodArgumentTypeMismatchException e) {
        log.warn("[common:exception] 参数类型错误, {}", e.getMessage());
    ...
    public void handleIllegalState(IllegalStateException e, HttpServletResponse response) {
        log.error("[common:exception] 响应状态异常, 直接写入错误响应", e);
        if (response.isCommitted()) return;
        try {
            HttpResponseUtil.writeJson(response, 500, 500, "系统错误");
        } catch (Exception ex) {
            log.warn("[common:exception] 写入错误响应失败", ex);
        }
    }

    @ExceptionHandler(Exception.class)
    public Result<?> handleException(Exception e) {
        log.error("[common:exception] 未捕获异常", e);
