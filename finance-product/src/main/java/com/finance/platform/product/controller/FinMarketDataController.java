...
public class FinMarketDataController {
    ...
    public Result<FinMarketDataVO> getById(@PathVariable Long id) {
        return Result.success(finMarketDataService.getMarketDataById(id));
    }
    ...
    @PostMapping
    public Result<Boolean> create(@RequestBody FinMarketDataDTO dto) {
        return Result.success(finMarketDataService.createMarketData(dto));
    }

    @Operation(summary = "更新行情数据")
    @PutMapping("/{id}")
    public Result<Boolean> update(@PathVariable Long id, @RequestBody FinMarketDataDTO dto) {
