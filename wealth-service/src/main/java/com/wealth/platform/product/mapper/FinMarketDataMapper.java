...
public interface FinMarketDataMapper extends BaseMapper<WeaMarketData> {
    ...
    /** 查询全部产品价格总和（用于仪表盘总资产） */
    @Select("SELECT COALESCE(SUM(price), 0) FROM wea_market_data")
    BigDecimal sumPrice();

    /** 查询最近两条有价格记录的行情数据（用于计算资产变化率） */
    @Select("SELECT price FROM wea_market_data WHERE price IS NOT NULL ORDER BY create_time DESC LIMIT 2")
